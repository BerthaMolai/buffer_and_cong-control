...
public class Graph
{
    ...
    /**
     * Method to get next hop from available path
     * calls a recursive routine to add shortest path to destNode to a list
     * @param destName
     */
    public String checkPath( String destName )
    {
        trip.clear();
        ...
        else
        {
            ...
            String nextHop;
            if(trip.size() > 2 ){
                nextHop = trip.get(2); //return nextHop
            }
            else
                nextHop = null;
     ...
     public void printPath( String destName)
    {
        ...
        else
        {
               ...
               printPath( w );
               System.out.print("cost = " + getCost(destName));  
               System.out.println();
     ...
     * is known to exist.
     */
    private void checkPath(Vertex dest )
    {
        if( dest.prev != null )
        {
            //trip.add(dest.prev.name);
            checkPath( dest.prev );
            trip.add(dest.prev.name);
            //System.out.print( "to " );
        }
    ...
    /**
     * to update link cost when required
     * @param src   source router
     * @param destName  destination router
     * @param new_d     new cost
     */
    public void changeCost(String src, String destName, double qdiff){
        Vertex v = vertexMap.get(src);
        Vertex w = vertexMap.get(destName);
        //iterate through adj list of v
        for(Edge e: v.adj){
            if(e.dest.equals(w)){
                //change cost 
                double oldc = e.cost;
                ...
                System.out.println("Cost from " + src + " to " +destName +" updated from "+ oldc + " to " + e.cost );
                return;
            }
        }
        //w.dist = new_d;
    ...
    public void dijkstra( String startName )
    {
        ...
        while( !pq.isEmpty( ) && nodesSeen < vertexMap.size( ) )
        {
            ...
            for( Edge e : v.adj )
            {
                ...
                if( w.dist > v.dist + cvw ) //we want to have >=, and ability to store multiple paths
                {
                    w.dist = v.dist +cvw;
                    w.prev = v;
